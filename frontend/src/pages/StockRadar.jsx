...
function DesktopStockRow({ stock }) {
  return (
    <tr className="border-t border-gray-800/60 bg-gray-900/30 hover:bg-gray-800/40">
      <td className="py-2 pl-10 pr-3">
        <div className="flex flex-col">
          <span className="text-xs font-medium text-gray-200">{stock.label}</span>
          <span className="text-[10px] text-gray-600">{stock.symbol?.replace(".NS", "")}</span>
        </div>
      </td>
      {WINDOWS.map(({ key, range }) => (
        <RetCell key={key} value={stock[key]} range={range} />
      ))}
      <td className="px-3 py-2 text-center tabular-nums text-xs text-gray-400">
        {stock.z1w != null ? stock.z1w.toFixed(2) : "—"}
      </td>
...
function DesktopSectorRow({ sec, isExpanded, onToggle }) {
  const { sector, median: m, stocks, stockCount } = sec;
      ...
      >
        <td className="px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-gray-100">{sector}</span>
            {badge && (
              <span className={`rounded px-1.5 py-0.5 text-[10px] font-bold ${badge.cls}`}>
                {badge.label}
              </span>
            )}
            <span className="text-xs text-gray-600">({stockCount ?? stocks?.length} stocks)</span>
          </div>
        </td>
        {WINDOWS.map(({ key, range }) => (
        ...
        <td className="px-3 py-3 text-right text-xs text-gray-600">—</td>
      </tr>
      {isExpanded && stocks.map((s) => (
        <DesktopStockRow key={s.symbol} stock={s} />
      ))}
    </>
  );
}

function HotColdStrip({ sectors }) {
  const sorted = [...sectors].sort((a, b) => (b.median.z1w ?? -Infinity) - (a.median.z1w ?? -Infinity));
  const hot  = sorted.slice(0, 3).filter((s) => (s.median.z1w ?? 0) > 0);
  const cold = sorted.slice(-3).filter((s) => (s.median.z1w ?? 0) < 0).reverse();
      ...
      {hot.length > 0 && (
        <div className="rounded-xl border border-green-800/50 bg-green-900/20 p-4">
          <div className="mb-2 text-xs font-semibold uppercase tracking-wider text-green-400">
            🔥 Strongest sector momentum this week
          </div>
          {hot.map((s) => (
            <div key={s.sector} className="flex items-center justify-between py-1">
              ...
              <div className="flex items-center gap-3">
                <span className="tabular-nums text-sm font-semibold text-green-400">{fmt(s.median.ret1w)} 1W</span>
                <span className="text-xs text-gray-500">z={s.median.z1w?.toFixed(1)}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {cold.length > 0 && (
        <div className="rounded-xl border border-red-900/50 bg-red-900/20 p-4">
          <div className="mb-2 text-xs font-semibold uppercase tracking-wider text-red-400">
            ❄ Weakest sector momentum this week
          </div>
          {cold.map((s) => (
            <div key={s.sector} className="flex items-center justify-between py-1">
              <span className="text-sm text-gray-200">{s.sector}</span>
              <div className="flex items-center gap-3">
                <span className="tabular-nums text-sm font-semibold text-red-400">{fmt(s.median.ret1w)} 1W</span>
                <span className="text-xs text-gray-500">z={s.median.z1w?.toFixed(1)}</span>
              </div>
            </div>
          ))}
...
function Legend() {
  return (
    <div className="mb-4 flex flex-wrap items-center gap-4 text-xs text-gray-500">
      ...
      <span>z ≥ 2 = unusually strong week · z ≤ -2 = unusually weak week</span>
      <span>Click a sector row to expand individual stocks</span>
    </div>
  );
}
    ...
    <>
      <div className="mb-6">
        <div className="flex flex-wrap items-baseline justify-between gap-2">
          <div>
            <h1 className="text-2xl font-bold">Stock Momentum Radar</h1>
            <p className="mt-1 text-sm text-gray-400">
              Median price returns across {data.sectors.length} NSE sectors · updated {builtAt ? timeAgo(builtAt) : "—"}
            </p>
          </div>
        </div>
      </div>

      <HotColdStrip sectors={data.sectors} />
      <Legend />
          ...
          <thead className="bg-gray-800/80">
            <tr>
              ...
              <th
                ...
                className="cursor-pointer px-3 py-3 text-center text-xs font-semibold uppercase tracking-wider text-gray-400 hover:text-white"
                title="1-week z-score vs trailing 90-day weekly distribution"
              >
                z<SortArrow col="z1w" />
              </th>
          ...
          <tbody>
            {sorted.map((sec) => (
              <DesktopSectorRow
                key={sec.sector}
                sec={sec}
                isExpanded={expanded.has(sec.sector)}
                onToggle={() => toggleExpand(sec.sector)}
              />
            ))}
          </tbody>
        </table>
      ...
      {data.warnings?.length > 0 && (
        <details className="mt-6 text-xs text-gray-600">
          <summary className="cursor-pointer hover:text-gray-400">
            {data.warnings.length} stock(s) skipped
          </summary>
      ...
      <div className="flex gap-3 overflow-x-auto">
        {MOBILE_WINDOWS.map((w, i) => (
          <div key={w} className="shrink-0 text-center">
            ...
            <RetVal val={stock[MOBILE_WINDOW_KEYS[i]]} />
          </div>
        ))}
      </div>
    </div>
...
function MobileSectorCard({ sec }) {
  const [open, setOpen] = useState(false);
  const zl = zLabel(sec.median.z1w);
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            ...
            {sec.stocks[0] && (
              <span className="text-xs text-gray-500">
                Top: {sec.stocks[0].label}
                {sec.stocks[0].price ? ` · ₹${sec.stocks[0].price.toLocaleString("en-IN")}` : ""}
              </span>
            )}
          </div>
          <div className="flex shrink-0 items-center gap-3">
            <div className="text-right">
              ...
              <RetVal val={sec.median.ret1w} />
            </div>
            <div className="text-right">
              <div className="text-xs text-gray-600">1M</div>
              <RetVal val={sec.median.ret1m} />
            </div>
        ...
        <div className="border-t border-gray-800">
          <div className="flex gap-3 overflow-x-auto bg-gray-800/40 px-4 py-2">
            ...
            {MOBILE_WINDOWS.map((w, i) => (
              <div key={w} className="shrink-0 text-center">
                <div className="text-[10px] text-gray-600">{w}</div>
                <RetVal val={sec.median[MOBILE_WINDOW_KEYS[i]]} />
              </div>
            ))}
          </div>
          <div className="px-4">
            {sec.stocks.map((s) => <MobileStockRow key={s.symbol} stock={s} />)}
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-xl font-bold">Stock Radar</h1>
          <p className="mt-0.5 text-xs text-gray-500">
            {data.sectors.length} sectors · updated {builtAt ? timeAgo(builtAt) : "—"}
          </p>
        </div>
      </div>
      {data.sectors.map((sec) => (
        <MobileSectorCard key={sec.sector} sec={sec} />
      ))}
    </div>
  );
}
